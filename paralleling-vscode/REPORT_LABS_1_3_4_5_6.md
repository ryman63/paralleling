...
**Тема серии работ:** Реализация и исследование последовательных и параллельных версий вычислительного конвейера (Generate → Map → Merge → Sort → Reduce).

**Студент:** _________________________________________________________ (ФИО, группа)
...
Все эксперименты выполнялись на персональном компьютере с процессором **AMD Ryzen 5 5600X**. Процессор содержит 6 физических ядер и поддерживает 12 логических потоков, базовая тактовая частота составляет 3.7 ГГц, максимальная частота в режиме Boost — до 4.6 ГГц. Архитектура системы — 64-битная.
...
## 6. Лабораторная работа №3 — распараллеливание циклов с использованием директив
...
## 7. Лабораторная работа №4 — доверительные интервалы и параллельная сортировка
...
## 8. Лабораторная работа №5 — использование POSIX Threads
...
## 9. Лабораторная работа №6 — использование OpenCL
...
Также была учтена ограниченная поддержка двойной точности на некоторых устройствах: в таких случаях реализован fallback на вычисления в формате float или выполнение на CPU. Для практического использования рекомендуется реализовать автоматический выбор режима выполнения на основе профилирования и характеристик устройства.
...
Для получения корректных и воспроизводимых результатов рекомендуется фиксировать программную среду, выполнять прогревочные прогоны, использовать серии измерений (не менее пяти запусков) и приводить в отчётах средние значения, стандартное отклонение и доверительные интервалы.

---

## Приложение — шаблоны команд для сборки и запуска

**Сборка:**

```
gcc <source.c> -O2 -fopenmp -pthread -lOpenCL -o <executable>
```

**Запуск:**

```
<executable> <N> <THREADS> [opencl]
```
